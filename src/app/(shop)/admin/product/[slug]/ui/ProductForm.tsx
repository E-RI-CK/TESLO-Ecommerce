...
import { Category, CategoryItem, Product, ProductImage as ProductWithImage } from "@/interfaces";
...
export const ProductForm = ({ product, categories }: Props) => {
    ...
    watch('sizes'); //Rerenderizar formulario cuando cambia sizes

    const onSizeChanged = (size: string) => {

        let newSizes = getValues('sizes');

        if (newSizes.includes(size)) {
            newSizes = newSizes.filter(sizeItem => sizeItem !== size);
        }
        else {
            newSizes.push(size);
        }
...

                        {
                            sizes.map(size => (
                                ...
                                <div key={size}
                                    ...
                                    className={clsx(
                                        "p-2 border cursor-pointer rounder-md mr-2 mb-2 w-14 transition-all text-center",
                                        {
